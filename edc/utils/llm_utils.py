...
# Create a thread-local storage for API keys
thread_local = threading.local()

# Keep track of API key usage and errors
class ApiKeyManager:
    def __init__(self):
...
def openai_chat_completion_with_key(model, messages, temperature=0, max_tokens=512, retry_count=0):
    ...
    try:
        ...
        # Get a new key if rate limited or after several retries
...
def openai_chat_completion(model, system_prompt, history, temperature=0, max_tokens=512):
    """Make an OpenAI chat completion request with automatic key rotation"""
...
def batch_openai_chat_completion(model, system_prompt, batch_histories, temperature=0, max_tokens=512, max_workers=5):
    ...
    # Prepare all the message objects first
    all_messages = []
    ...
    # Function to process a single request and store the result
